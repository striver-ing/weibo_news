...
class Collector(threading.Thread):
    def __init__(self, tab_urls, site_parsers):
    ...
    def __input_data(self):
        ...
        urls_list = []
        if self._depth:
            urls_list = self._db.find(self._tab_urls, {"status": Constance.TODO, "site_id": {'$in': self._site_parsers},
                                                       "depth": {"$lte": self._depth}}, limit=url_count)
        else:
            urls_list = self._db.find(self._tab_urls,
                                      {"status": Constance.TODO, "site_id": {'$in': self._site_parsers}},
                                      limit=url_count)
